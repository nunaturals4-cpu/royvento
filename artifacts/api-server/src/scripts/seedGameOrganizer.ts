/**
 * Seeds a fully-detailed demo Game Organizer (venue + games + packages) so the
 * Game Management dashboard and public profile can be explored end-to-end.
 *
 * Run:  pnpm --filter @workspace/api-server exec tsx --env-file=.env.local src/scripts/seedGameOrganizer.ts
 *
 * Idempotent: re-running upserts the same account by email and re-creates the
 * games/packages only if the organizer has none yet.
 */
import {
  db,
  usersTable,
...
const EMAIL = "gamezone@royvento.com";
const PASSWORD = "Game@1234";
...
async function main() {
  ...
  if (user) {
    ...
    [user] = await db.insert(usersTable).values({
      email: EMAIL, name: "Neon Arcade Owner", passwordHash, role: "game_organizer",
  ...
  // Covers are matched to each game and verified to return HTTP 200.
  const games = [
  ...
  logger.info({ userId: user.id, organizerId: org.id, slug: org.slug }, "Game organizer seeded");
  // eslint-disable-next-line no-console
  console.log(`\n✅ Game Organizer seeded.\n   Login email:    ${EMAIL}\n   Login password: ${PASSWORD}\n   Dashboard:      /dashboard/game-organizer\n   Public page:    /game-organizers/${org.slug}\n`);
  process.exit(0);
}
