import { Ionicons } from "@expo/vector-icons";
import { LinearGradient } from "expo-linear-gradient";
import { router, useLocalSearchParams, Redirect } from "expo-router";
import {
  getGetCategorySummaryQueryKey,
  getGetLocalitySummaryQueryKey,
  useGetCategorySummary,
  useGetLocalitySummary,
  type VendorSummary,
} from "@workspace/api-client-react";
import React, { useState } from "react";
import {
  ActivityIndicator,
  Platform,
  Pressable,
  ScrollView,
  StyleSheet,
  Text,
  View,
} from "react-native";
import { useSafeAreaInsets } from "react-native-safe-area-context";
import { CrossLinkRail } from "@/components/CrossLinkRail";
import { EventCard } from "@/components/EventCard";
import { MobileFooter } from "@/components/MobileFooter";
import { BOTTOM_NAV_HEIGHT } from "@/components/PersistentBottomNav";
import { useColors } from "@/hooks/useColors";
import {
  PUB_CATEGORY_SLUGS,
  buildCategoryFAQs,
  buildLocalityFAQs,
  canonicalCitySlug,
  findCategoryBySlug,
  isAliasedCity,
  isCategorySlug,
  slugify,
  titleCase,
} from "@/lib/seoSlug";

const THIN_THRESHOLD = 4;

export default function CitySecondaryScreen() {
  const colors = useColors();
  const insets = useSafeAreaInsets();
  const { city: rawCity, locality: rawSecond } = useLocalSearchParams<{
    city: string;
    locality: string;
  }>();
  const citySlug = canonicalCitySlug(rawCity ?? "");
  const cityName = titleCase(citySlug);
  const second = rawSecond ?? "";
  const isCategory = isCategorySlug(second);
  const category = isCategory ? findCategoryBySlug(second) : null;
  const localitySlug = !isCategory ? slugify(second) : "";
  const localityName = localitySlug ? titleCase(localitySlug) : "";
  const topPadding = Platform.OS === "web" ? 67 : insets.top;
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  type CategorySlugParam = Parameters<typeof useGetCategorySummary>[1];
  const categorySlug = (isCategory && category ? category.slug : "pubs") as CategorySlugParam;

  const {
    data: catSummary,
    isLoading: catLoading,
    isError: catError,
  } = useGetCategorySummary(citySlug, categorySlug, {
    query: {
      queryKey: getGetCategorySummaryQueryKey(citySlug, categorySlug),
      enabled: !!citySlug && isCategory,
      staleTime: 5 * 60 * 1000,
    },
  });
  const {
    data: locSummary,
    isLoading: locLoading,
    isError: locError,
  } = useGetLocalitySummary(citySlug, localitySlug, {
    query: {
      queryKey: getGetLocalitySummaryQueryKey(citySlug, localitySlug),
      enabled: !!citySlug && !isCategory && !!localitySlug,
      staleTime: 5 * 60 * 1000,
    },
  });

  if (rawCity && isAliasedCity(rawCity) && rawCity !== citySlug) {
    return <Redirect href={`/city/${citySlug}/${slugify(second)}`} />;
  }

  if (!citySlug || !second) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.background, alignItems: "center", justifyContent: "center" }}>
        <Text style={{ color: colors.mutedForeground }}>Page not found</Text>
      </View>
    );
  }

  const isLoading = isCategory ? catLoading : locLoading;
  const isErr = isCategory ? catError : locError;
  const summary = isCategory ? catSummary : locSummary;

  if (isLoading) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.background, alignItems: "center", justifyContent: "center" }}>
        <ActivityIndicator color={colors.primary} />
      </View>
    );
  }

  if (isErr || !summary) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.background, alignItems: "center", justifyContent: "center", padding: 20 }}>
        <Text style={{ color: colors.mutedForeground, textAlign: "center" }}>Couldn't load this page.</Text>
      </View>
    );
  }

  const topPubs: VendorSummary[] = summary.topVendors ?? [];
  const count = summary.vendorCount ?? 0;
  const isThin = count < THIN_THRESHOLD;

  const subjectLabel = isCategory ? category!.label : "Pubs";
  const subjectName = isCategory ? category!.label : localityName;
  const h1 = isCategory
    ? `${count || ""} Best ${category!.label} in ${cityName}`.trim()
    : `Best Pubs in ${localityName}, ${cityName}`;
  const introCopy = isCategory
    ? `Hand-picked ${category!.label.toLowerCase()} in ${cityName} with instant table booking, photos, ratings and current offers. Filter further by locality, free-entry days and drink deals.`
    : `Top pubs and party venues in ${localityName}, ${cityName} with verified ratings, instant booking and weekly offers.`;
  const faqs = isCategory
    ? buildCategoryFAQs(category!.label, cityName)
    : buildLocalityFAQs(localityName, cityName);

  const otherCategories = PUB_CATEGORY_SLUGS.filter(
    (c) => !isCategory || c.slug !== category!.slug,
  )
    .slice(0, 8)
    .map((c) => ({
      href: `/city/${citySlug}/${c.slug}`,
      label: `${c.label} in ${cityName}`,
    }));

  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <LinearGradient
        colors={[colors.card, colors.background]}
        style={[styles.header, { paddingTop: topPadding + 12, borderBottomColor: colors.border }]}
      >
        <Pressable style={styles.backBtn} onPress={() => router.back()}>
          <Ionicons name="arrow-back" size={22} color={colors.foreground} />
        </Pressable>
        <View style={styles.crumb}>
          <Pressable onPress={() => router.push("/(tabs)")}>
            <Text style={[styles.crumbItem, { color: colors.mutedForeground }]}>Home</Text>
          </Pressable>
          <Ionicons name="chevron-forward" size={11} color={colors.mutedForeground} />
          <Pressable onPress={() => router.push(`/city/${citySlug}`)}>
            <Text style={[styles.crumbItem, { color: colors.mutedForeground }]}>{cityName}</Text>
          </Pressable>
          <Ionicons name="chevron-forward" size={11} color={colors.mutedForeground} />
          <Text style={[styles.crumbCurrent, { color: colors.foreground }]} numberOfLines={1}>{subjectName}</Text>
        </View>
      </LinearGradient>

      <ScrollView
        showsVerticalScrollIndicator={false}
        contentContainerStyle={[styles.scroll, { paddingBottom: BOTTOM_NAV_HEIGHT + insets.bottom + 24 }]}
      >
        <View style={styles.hero}>
          <View style={[styles.heroBadge, { borderColor: colors.primary + "55", backgroundColor: colors.primary + "1A" }]}>
            <Ionicons name="location" size={12} color={colors.primary} />
            <Text style={[styles.heroBadgeText, { color: colors.primary }]}>{cityName}</Text>
          </View>
          <Text style={[styles.heroTitle, { color: colors.foreground }]}>{h1}</Text>
          <Text style={[styles.heroDesc, { color: colors.mutedForeground }]}>{introCopy}</Text>
        </View>

        {isThin ? (
          <View style={[styles.thinCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <Text style={[styles.thinTitle, { color: colors.foreground }]}>
              We're still adding {subjectLabel.toLowerCase()}{!isCategory ? " in this area" : ""}
            </Text>
            <Text style={[styles.thinDesc, { color: colors.mutedForeground }]}>
              Try all pubs in {cityName} or browse all pubs.
            </Text>
            <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap", justifyContent: "center" }}>
              <Pressable
                onPress={() => router.push(`/city/${citySlug}`)}
                style={[styles.thinCta, { backgroundColor: colors.primary }]}
              >
                <Text style={[styles.thinCtaText, { color: colors.primaryForeground }]}>All pubs in {cityName}</Text>
              </Pressable>
              <Pressable
                onPress={() => router.push("/(tabs)/pubs")}
                style={[styles.thinCta, { backgroundColor: colors.muted, borderWidth: 1, borderColor: colors.border }]}
              >
                <Text style={[styles.thinCtaText, { color: colors.foreground }]}>Browse all pubs</Text>
              </Pressable>
            </View>
            {topPubs.length > 0 ? (
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 16, marginHorizontal: -20 }}>
                <View style={{ flexDirection: "row", paddingHorizontal: 20, gap: 12 }}>
                  {topPubs.map((v) => (
                    <EventCard
                      key={v.id}
                      id={v.id}
                      vendorId={v.id}
                      title={v.businessName}
                      type="pub"
                      location={[v.city, v.state].filter(Boolean).join(", ")}
                      imageUrl={v.bannerImage ?? undefined}
                      rating={v.rating}
                      reviewCount={v.reviewCount}
                      directBooking
                    />
                  ))}
                </View>
              </ScrollView>
            ) : null}
          </View>
        ) : (
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { color: colors.foreground }]}>
              Top picks{!isCategory ? ` in ${localityName}` : ""}
            </Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginHorizontal: -16 }}>
              <View style={{ flexDirection: "row", paddingHorizontal: 16, gap: 12 }}>
                {topPubs.map((v) => (
                  <EventCard
                    key={v.id}
                    id={v.id}
                    vendorId={v.id}
                    title={v.businessName}
                    type="pub"
                    location={[v.city, v.state].filter(Boolean).join(", ")}
                    imageUrl={v.bannerImage ?? undefined}
                    rating={v.rating}
                    reviewCount={v.reviewCount}
                    directBooking
                  />
                ))}
              </View>
            </ScrollView>
          </View>
        )}

        {otherCategories.length > 0 ? (
          <View style={styles.section}>
            <CrossLinkRail
              title={isCategory ? "Other categories" : `Also see categories in ${cityName}`}
              links={otherCategories}
            />
          </View>
        ) : null}

        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.foreground }]}>Frequently asked questions</Text>
          <View style={{ gap: 8 }}>
            {faqs.map((f, i) => {
              const open = openFaq === i;
              return (
                <Pressable
                  key={f.question}
                  onPress={() => setOpenFaq(open ? null : i)}
                  style={[styles.faqCard, { backgroundColor: colors.card, borderColor: colors.border }]}
                >
                  <View style={styles.faqHeader}>
                    <Text style={[styles.faqQuestion, { color: colors.foreground }]}>{f.question}</Text>
                    <Ionicons
                      name={open ? "chevron-up" : "chevron-down"}
                      size={16}
                      color={colors.mutedForeground}
                    />
                  </View>
                  {open ? (
                    <Text style={[styles.faqAnswer, { color: colors.mutedForeground }]}>{f.answer}</Text>
                  ) : null}
                </Pressable>
              );
            })}
          </View>
        </View>

        <MobileFooter />
      </ScrollView>
    </View>
  );
}

const styles = StyleSheet.create({
  header: {
    paddingHorizontal: 16,
    paddingBottom: 12,
    borderBottomWidth: 1,
    gap: 8,
  },
  backBtn: { padding: 4, alignSelf: "flex-start" },
  crumb: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
    paddingHorizontal: 4,
    flexWrap: "wrap",
  },
  crumbItem: { fontSize: 12, fontFamily: "Inter_500Medium" },
  crumbCurrent: { fontSize: 12, fontFamily: "Inter_600SemiBold", flexShrink: 1 },
  scroll: { paddingTop: 4 },
  hero: { paddingHorizontal: 20, paddingVertical: 20, gap: 10 },
  heroBadge: {
    flexDirection: "row",
    alignItems: "center",
    gap: 6,
    alignSelf: "flex-start",
    borderRadius: 999,
    borderWidth: 1,
    paddingHorizontal: 10,
    paddingVertical: 4,
  },
  heroBadgeText: {
    fontSize: 10,
    fontFamily: "Inter_700Bold",
    textTransform: "uppercase",
    letterSpacing: 1.4,
  },
  heroTitle: {
    fontSize: 26,
    fontFamily: "Inter_700Bold",
    letterSpacing: -0.5,
    lineHeight: 32,
  },
  heroDesc: {
    fontSize: 14,
    fontFamily: "Inter_400Regular",
    lineHeight: 22,
  },
  section: { paddingHorizontal: 20, paddingVertical: 14, gap: 12 },
  sectionTitle: {
    fontSize: 18,
    fontFamily: "Inter_700Bold",
    letterSpacing: -0.2,
  },
  thinCard: {
    marginHorizontal: 20,
    marginVertical: 14,
    borderRadius: 18,
    borderWidth: 1,
    padding: 24,
    alignItems: "center",
    gap: 12,
  },
  thinTitle: {
    fontSize: 18,
    fontFamily: "Inter_700Bold",
    textAlign: "center",
  },
  thinDesc: {
    fontSize: 13,
    fontFamily: "Inter_400Regular",
    textAlign: "center",
    lineHeight: 19,
  },
  thinCta: {
    paddingHorizontal: 16,
    paddingVertical: 10,
    borderRadius: 999,
  },
  thinCtaText: {
    fontSize: 13,
    fontFamily: "Inter_600SemiBold",
  },
  faqCard: {
    borderRadius: 14,
    borderWidth: 1,
    padding: 14,
    gap: 8,
  },
  faqHeader: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
    gap: 8,
  },
  faqQuestion: {
    flex: 1,
    fontSize: 14,
    fontFamily: "Inter_600SemiBold",
    lineHeight: 19,
  },
  faqAnswer: {
    fontSize: 13,
    fontFamily: "Inter_400Regular",
    lineHeight: 19,
  },
});
