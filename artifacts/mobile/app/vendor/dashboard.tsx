...
export default function VendorDashboardScreen() {
  ...
  function renderDayTimePicker() {
    if (!activeDayPicker) return null;
  ...
  function renderVenueDetailsForm() {
    return (
      <>
    ...
    return (
      <ScrollView contentContainerStyle={[styles.list, { paddingBottom: 120 }]}>
        ...
        <View style={{ flexDirection: "row", gap: 8, marginBottom: 12 }}>
          {earningsKpis.map((k) => (
            <View key={k.label} style={[{ flex: 1, borderRadius: 14, borderWidth: 1, padding: 12, gap: 4, alignItems: "center" }, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <View style={[{ width: 32, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center" }, { backgroundColor: k.color + "20" }]}>
                <Ionicons name={k.icon} size={15} color={k.color} />
              </View>
              <Text style={{ fontSize: 14, fontFamily: "Inter_700Bold", color: k.color }}>{k.value}</Text>
              <Text style={{ fontSize: 10, fontFamily: "Inter_400Regular", color: colors.mutedForeground, textAlign: "center" }}>{k.label}</Text>
            </View>
          ))}
        </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
            <ScrollView contentContainerStyle={[styles.list, { paddingBottom: 80 }]}>
              ...
              <View style={{ borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 16, gap: 12, marginTop: 4 }}>
                <Text style={{ fontSize: 15, fontFamily: "Inter_700Bold", color: colors.foreground }}>Venue Details</Text>
      ...
      {/* ── Venue Details Modal (for vendors with pub already registered) ── */}
      <Modal visible={showVenueDetailsModal} animationType="slide" presentationStyle="pageSheet">
        <View style={{ flex: 1, backgroundColor: colors.background }}>
          <View style={[styles.modalHeader, { borderBottomColor: colors.border, backgroundColor: colors.card }]}>
            <TouchableOpacity onPress={() => setShowVenueDetailsModal(false)}>
              <Ionicons name="close" size={22} color={colors.foreground} />
            </TouchableOpacity>
            <Text style={[styles.modalTitle, { color: colors.foreground }]}>Venue Details</Text>
            <View style={{ width: 40 }} />
          </View>
