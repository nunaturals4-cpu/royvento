// Split Expense — client-side bill-splitting (Splitwise-style), React Native.
//
// Mirror of the web lib/splitExpense.ts. All data lives on-device via
// AsyncStorage (no backend table yet). Groups are namespaced per-user so two
// accounts on the same device don't share splits. Settlement math works in
// integer paise to avoid float drift, then renders back to rupees.
...
export async function loadGroups(userKey: string | number): Promise<SplitGroup[]> {
  try {
...
export async function saveGroups(userKey: string | number, groups: SplitGroup[]): Promise<void> {
  try {
    await AsyncStorage.setItem(storageKey(userKey), JSON.stringify(groups));
  } catch {
    /* quota / storage failure — best effort only */
  }
}
 ...
 *  > 0  → the group owes them money (creditor)
 *  < 0  → they owe the group money (debtor)
...
/**
 * Format an amount as INR with Indian digit grouping (1,23,456.78). Hand-rolled
 * so it works identically under Hermes, whose Intl support is limited.
 */
